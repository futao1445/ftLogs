...
const appRouter = t.router({
  ...
  llm: t.router({
    models: t.procedure
      .input(z.object({ provider: z.string().optional(), apiKey: z.string().optional(), apiUrl: z.string().optional() }))
      .query(async ({ input }) => {
        const readCfg = (key) => prisma.config.findUnique({ where: { key } }).then(c => {
          if (!c) return null; try { return JSON.parse(c.value); } catch { return c.value; }
        }).catch(() => null);
        ...
        const [apiUrl, apiKey] = await Promise.all([
          input.apiUrl || KNOWN_URLS[activeProvider] || readCfg().then(v => v || ''),
          input.apiKey || readCfg(),
        ]);
        if (!apiUrl || !apiKey) return { models: [], error: '请先配置 LLM' };
        try {
          const res = await fetch(, {
            headers: { 'Authorization':  }, signal: AbortSignal.timeout(10000),
          });
          if (!res.ok) return { models: [], error:  };
          const json = await res.json();
    ...
    test: t.procedure
      .input(z.object({ apiUrl: z.string(), apiKey: z.string(), model: z.string() }))
      .mutation(async ({ input }) => {
        ...
        try {
          const res = await fetch(, {
            ...
            body: JSON.stringify({ model: input.model, messages: [{ role: 'user', content: 'Reply with just "ok".' }], max_tokens: 10 }),
            signal: AbortSignal.timeout(15000),
          });
          if (!res.ok) { const bd = await res.text().catch(() => ''); return { success: false, error: , latency: Date.now() - start }; }
          return { success: true, model: input.model, latency: Date.now() - start };
        } catch (e) { return { success: false, error: e.message || 'Connection failed', latency: Date.now() - start }; }
      }),
    chat: t.procedure
      .input(z.object({ messages: z.array(z.object({ role: z.string(), content: z.string() })), provider: z.string().optional() }))
      .mutation(async ({ input }) => {
        const readCfg = (key) => prisma.config.findUnique({ where: { key } }).then(c => {
          if (!c) return null; try { return JSON.parse(c.value); } catch { return c.value; }
        }).catch(() => null);
        ...
        const KNOWN_URLS = { deepseek: 'https://api.deepseek.com', kimi: 'https://api.moonshot.cn/v1', aliyun: 'https://dashscope.aliyuncs.com/compatible-mode/v1' };
        const [apiUrl, apiKey, model] = await Promise.all([
          KNOWN_URLS[activeProvider] ? Promise.resolve(KNOWN_URLS[activeProvider]) : readCfg().then(v => v || 'https://api.openai.com/v1'),
          readCfg(),
          readCfg().then(v => v || 'gpt-4o-mini'),
        ]);
        if (!apiKey) return { success: false, error: 'LLM 未配置' };
        try {
          const res = await fetch(, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization':  },
          ...
          if (!res.ok) { const bd = await res.text().catch(() => ''); return { success: false, error:  }; }
          const json = await res.json();
          return { success: true, content: json.choices?.[0]?.message?.content || '' };
        } catch (e) { return { success: false, error: e.message || 'LLM 请求失败' }; }
      }),
  }),
});
